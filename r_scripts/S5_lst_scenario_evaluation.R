# =====================================================================
# Script 5 – LST Scenario Evaluation
# =====================================================================
...
if (length(lst_files) == 0) {
  stop("No finalized LST rasters found.")
}

if (length(obs_files) == 0) {
  stop("No finalized observation-count rasters found.")
}

# Extract scenario names
lst_scenarios <- gsub(
  "median_LST_FINAL_|\\.tif",
  "",
  basename(lst_files)
)

obs_scenarios <- gsub(
  "observation_count_FINAL_|\\.tif",
  "",
  basename(obs_files)
)

# Match observation rasters to LST rasters by scenario name
obs_files <- obs_files[match(lst_scenarios, obs_scenarios)]

if (any(is.na(obs_files))) {
  stop("At least one LST raster has no corresponding observation-count raster.")
}

# -----------------------------
# 3. Load census grid for Berlin created in Workflow 4
# -----------------------------
grid_file <- file.path(
  "results",
  "interim_results",
...
outline_vect <- project(
  outline_vect,
  crs(rast(lst_files[1]))
)

# -----------------------------
# 4. Function to compute raster statistics 
# -----------------------------
compute_raster_stats <- function(
    lst_raster,
    obs_raster,
    scenario_name,
    outline_vect
  ...
  NA_percent <- round(
    100 * NA_pixels / total_pixels,
    2
  )
  
  # Observation stats
  median_obs <- round(
    median(obs_valid, na.rm = TRUE),
    2
  )
  
  coverage_5 <- round(
    100 * sum(obs_inside >= 5, na.rm = TRUE) /
      length(obs_inside),
    1
  )
  
  coverage_6 <- round(
    100 * sum(obs_inside >= 6, na.rm = TRUE) /
      length(obs_inside),
    1
  )
  ...
  # LST stats
  median_lst <- round(
    median(lst_valid, na.rm = TRUE),
    2
  )
  
  iqr_lst <- round(
    IQR(lst_valid, na.rm = TRUE),
    2
  )
...
write_csv(
  ...
  file.path(
    main_results_path,
    "5_1_table_lst_raster_statistics.csv"
  )
)

# -----------------------------
# 7. Spatial agreement with selected scenario (4y_10cc)
# -----------------------------
...
selected_idx <- which(
  lst_scenarios == selected_scenario
)

if (length(selected_idx) != 1) {
  stop("Selected LST scenario was not found or is not unique.")
}

# Load and mask selected raster
selected_raster <- rast(lst_files[selected_idx])
selected_masked <- mask(
  selected_raster,
  outline_vect
)
...
write_csv(
  ...
  file.path(
    appendix_path,
    "appendix_table_lst_spatial_agreement_to_4y_10cc.csv"
  )
)