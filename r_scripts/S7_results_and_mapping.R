...
berlin <- st_read(
  file.path("study_data", "berlin.shp"),
  quiet = TRUE
)

grid <- st_read(file.path(
  "results", "interim_results", "berlin_grid_final.gpkg"
))

lst_raster_file <- file.path(
  "study_data", "median_LST_FINAL_4y_10cc.tif"
)
...
main_results_path <- file.path("results", "main_results")
appendix_path <- file.path("results", "appendix")
...
# Results were used to create Table 7 manually
...
# => match correlation results, not reported in thesis
