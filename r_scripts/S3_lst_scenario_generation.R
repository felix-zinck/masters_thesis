# =====================================================================
# Script 3 – Median LST Raster Generation
# =====================================================================
...
# Replace the placeholders with the Google account and Google Cloud project
# associated with your Earth Engine setup
ee_Initialize(
  user = "your_google_account@example.com",
...
# File paths
admin_shapefile <- file.path(
  "supplementary_data",
  "zensus_administrative_borders",
  "VG250_GEM.shp"
)
...
admin_borders_germany <- st_read(
  admin_shapefile,
  quiet = TRUE
)

# Extract Berlin polygon (GEN = attribute for geographic name)
berlin <- admin_borders_germany[
  admin_borders_germany$GEN == "Berlin",
]
...
# -----------------------------
hot_days_df <- read_csv(
  file.path(
    "results",
    "interim_results",
    "01_tempelhof_heat_days_2018_2025.csv"
  ),
...
dir.create(
  file.path(out_path, "median_LST_raw"),
  showWarnings = FALSE,
  recursive = TRUE
)

dir.create(
  file.path(out_path, "observation_count_raw"),
  showWarnings = FALSE,
  recursive = TRUE
)
...
# -----------------------------
# 10. Generate median LST and observation count rasters
# -----------------------------
...
# -----------------------------
lst_final_path <- file.path(
  out_path,
  "median_LST_final"
)

obs_final_path <- file.path(
  out_path,
  "observation_count_final"
)
...
for (lst_file in lst_files) {
  ...
  if (!file.exists(obs_file)) {
    warning(
      paste0(
        "Observation raster not found for scenario: ",
        scenario_name
      )
    )
    next
...
hot_dates_gee_check <- hot_days_df %>%
  dplyr::filter(
    lubridate::year(date) >=
      lubridate::year(max(date)) - ny_check + 1,
    lubridate::year(date) <=
...
# Recreate the filtered Landsat image collection for the check scenario and
...
scene_times_utc <- unlist(
  landsat_hot_check$
    aggregate_array("SCENE_CENTER_TIME")$
    getInfo()
)

if (length(scene_times_utc) == 0) {
  stop("No Landsat scene times found for the acquisition-time check.")
}
...
write_csv(
  landsat_time_summary,
  file.path(
    interim_results_path,
    "table_landsat_acquisition_times.csv"
  )
)