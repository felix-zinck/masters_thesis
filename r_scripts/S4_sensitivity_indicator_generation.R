# =====================================================================
# Script 4 – Sensitivity Layer Generation 
# =====================================================================
...
# -----------------------------
# 1. Add Zensus 2022 attributes from CSV files based on grid_id to Berlin grid 
# -----------------------------

# Define output paths
interim_results_path <- file.path(
  "results",
  "interim_results"
)
...
# -----------------------------
# 1.1 Load and preprocess Zensus 2022 attribute CSV files
# -----------------------------
...
names(zensus_list) <- tools::file_path_sans_ext(
  basename(csv_files)
)
...
write_csv(
  ...
  file.path(
    interim_results_path,
    "02_table_zensus_csv_summary.csv"
  )
)

# -------------------------------------------------
# 1.2 Add Zensus attributes to Berlin grid
# -------------------------------------------------
...
# -------------------------------------------------
# 1.3 Define attribute columns needed for indicator calculation and to confirm 
...
berlin_grid_sens_atr <- berlin_grid_atr %>%
...
sum(
  berlin_grid_sens_atr$population !=
    berlin_grid_sens_atr$population_check,
  na.rm = TRUE
)

sum(
  abs(
    berlin_grid_sens_atr$avg_age -
      berlin_grid_sens_atr$avg_age_check
  ) > 0.0001,
  na.rm = TRUE
)
...
# Output path
output_grid_sens_atr <- file.path(
  interim_results_path,
  "berlin_grid_sens_atr.gpkg"
)

st_write(
  berlin_grid_sens_atr,
  output_grid_sens_atr,
  delete_layer = TRUE,
  quiet = TRUE
)

# -----------------------------
# 2. Indicator calculation and normalization
# -----------------------------

# Load and read data
input_grid <- file.path(
  interim_results_path,
  "berlin_grid_sens_atr.gpkg"
)

berlin_grid_sens_atr <- st_read(
  input_grid,
  quiet = TRUE
)
...
berlin_grid_indicators_raw <- berlin_grid_sens_atr %>%
  mutate(
    # Demographic
    share_elderly_65plus = if_else(
      population > 0,
      elderly_65plus / population,
      NA_real_
    ),
    
    share_children_under6 = if_else(
      population > 0,
    ...
    # Social structure
    share_single_households = if_else(
      total_households > 0,
      single_person_households / total_households,
      NA_real_
    ),
    
    share_single_parent_families = if_else(
      total_families > 0,
      (
    ...
    share_foreign_born = if_else(
      population > 0,
      foreign_born / population,
      NA_real_
    ),
    
    share_non_citizens = if_else(
      population > 0,
    ...
    inv_avg_rent = if_else(
      !is.na(avg_rent_eur_m2) &
        avg_rent_eur_m2 > 0,
      1 / avg_rent_eur_m2,
      NA_real_
    ),
    
    inv_avg_living_space = if_else(
      !is.na(avg_living_space_m2) &
        avg_living_space_m2 > 0,
...
# Strip geometry
indicator_df <- st_set_geometry(
  berlin_grid_indicators_raw,
  NULL
)

# Select share indicator columns only
share_cols <- indicator_df %>%
...
denominator_lookup <- tibble::tibble(
  ...
  denominator_type = c(
    ...
    "families", "population", "population"
  ),
  denominator_col = c(
    "population", "population", "total_households",
    "total_families", "population", "population"
  )
)

# Create diagnostics table for share indicator grid cells out of logical bounds
share_ind_out_of_bounds <- purrr::map_dfr(
  share_cols,
  function(ind) {
    
    denominator_col <- denominator_lookup %>%
      filter(indicator == ind) %>%
      pull(denominator_col)
    
    denominator_type <- denominator_lookup %>%
      filter(indicator == ind) %>%
      pull(denominator_type)
    ...
    tibble::tibble(
      ...
      min_denominator = if (nrow(cells_gt1) > 0) {
        min(
          cells_gt1[[denominator_col]],
          na.rm = TRUE
        )
      } else {
        NA_real_
      },
      median_denominator = if (nrow(cells_gt1) > 0) {
        round(
          median(
            cells_gt1[[denominator_col]],
            na.rm = TRUE
      ...
      max_denominator = if (nrow(cells_gt1) > 0) {
        max(
          cells_gt1[[denominator_col]],
          na.rm = TRUE
        )
...
write_csv(
  ...
  file.path(
    appendix_path,
    "appendix_table_share_indicators_out_of_bounds.csv"
  )
)
...
summary(
  berlin_grid_indicators_cap %>%
    st_set_geometry(NULL) %>%
    select(all_of(share_cols))
)

# Save Berlin 100m grid containing capped heat sensitivity indicators
output_grid_indicators_cap <- file.path(
  interim_results_path,
  "berlin_grid_indicators_cap.gpkg"
)

st_write(
  berlin_grid_indicators_cap,
  output_grid_indicators_cap,
  delete_layer = TRUE,
  quiet = TRUE
)

# -----------------------------
# 3. Normalize indicators using min-max normalization
# -----------------------------
...
berlin_grid_indicators <- st_read(
  input_grid,
  quiet = TRUE
)
...
minmax_norm <- function(x) {
  ...
  if (rng[1] == rng[2]) {
    return(rep(NA_real_, length(x))) # Return NA if indicator
  }
...
# normalized indicators to the grid
...
norm_cols <- names(
  berlin_grid_indicators_norm
)[
  grepl(
    "_norm$",
    names(berlin_grid_indicators_norm)
  )
]
...
output_grid_indicators_norm <- file.path(
  interim_results_path,
  "berlin_grid_indicators_norm.gpkg"
)

st_write(
  berlin_grid_indicators_norm,
  output_grid_indicators_norm,
  delete_layer = TRUE,
  quiet = TRUE
)

# -----------------------------
# 4. Spearman: Correlation matrix between indicators to remove possible double counts
# -----------------------------
...
berlin_grid_indicators_norm <- st_read(
  input_grid,
  quiet = TRUE
)
...
norm_indicator_cols <- indicator_df %>%
  select(ends_with("_norm")) %>%
  names()

norm_indicators <- indicator_df %>%
  select(all_of(norm_indicator_cols))
...
colnames(cor_matrix) <- label_map[
  colnames(cor_matrix)
]

rownames(cor_matrix) <- label_map[
  rownames(cor_matrix)
]
...
png(
  filename = file.path(
    appendix_path,
    "appendix_figure_indicator_correlation_matrix.png"
  ),
  width = 120,
  height = 100,
  units = "mm",
...
# Decision which indicator to drop
# Non-citizen population indicator has more NAs and lower variance
sapply(
  indicator_df[
    ,
    c(
      "share_foreign_born_norm",
      "share_non_citizens_norm"
    )
  ],
...
# -----------------------------
# 5. Create and normalize composite heat sensitivity index using equal weighting (mean)
# -----------------------------
...
berlin_grid_indicators_norm_comp <- berlin_grid_indicators_norm %>%
  ...
  mutate(
    n_indicators = sum(
      !is.na(
        c(
          share_elderly_65plus_norm,
          share_children_under6_norm,
          share_single_households_norm,
          share_single_parent_families_norm,
          share_foreign_born_norm,
          inv_avg_rent_norm,
    ...
    composite_index = mean(
      ...
      na.rm = TRUE
    ) # Ensures mean is calculated only from valid values
  ) %>%
...
berlin_grid_indicators_norm_comp <- berlin_grid_indicators_norm_comp %>%
  mutate(
    composite_index_norm = minmax_norm(
      composite_index
    )
  )

# Check distribution of available indicators per grid cell
table(
  berlin_grid_indicators_norm_comp$n_indicators
)
...
output_grid_indicators_norm_comp <- file.path(
  interim_results_path,
  "berlin_grid_indicators_norm_comp.gpkg"
)

st_write(
  berlin_grid_indicators_norm_comp,
  output_grid_indicators_norm_comp,
  delete_layer = TRUE,
  quiet = TRUE
)