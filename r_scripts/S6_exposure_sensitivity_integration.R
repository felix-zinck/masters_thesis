# =====================================================================
# Script 6 – Merging sensitivity and exposure layer
# =====================================================================
...
lst_raster_file <- file.path(
  "study_data",
  "median_LST_FINAL_4y_10cc.tif"
)
...
# -----------------------------
if (!same.crs(lst_raster, st_crs(census_grid)$wkt)) {
  # Project raster to match Zensus grid (EPSG:3035)
  lst_raster <- terra::project(
    lst_raster,
    st_crs(census_grid)$wkt
  )
}
...
lst_cell_median <- terra::extract(
  lst_raster,
  census_vect,
  fun = function(x, ...) median(x, na.rm = TRUE)
)

lst_cell_mean <- terra::extract(
  lst_raster,
  census_vect,
...
valid_area_dt <- dt[
  ,
  .(lst_valid_area_m2 = sum(pixel_area_inside)),
  by = ID
]
...
minmax_norm <- function(x) {
  ...
  if (rng[1] == rng[2]) {
    return(rep(NA_real_, length(x))) # No variation => NA
  }
...
for (sei in sei_norm_cols) {
  ...
  census_grid[[esi_name]] <-
    census_grid[[sei]] + census_grid$lst_n_median
  
  census_grid[[paste0(esi_name, "_norm")]] <-
    minmax_norm(census_grid[[esi_name]])
}

# Calculate and normalize general ESI
census_grid$esi_comb_raw <-
  census_grid$composite_index_norm +
  census_grid$lst_n_median

census_grid$esi_n_comb <-
  minmax_norm(census_grid$esi_comb_raw)
...
census_grid_final <- census_grid_final %>%
  select(
    ...
    sei_r_elderly, sei_r_children, sei_r_single_hh,
    sei_r_single_parent, sei_r_foreign_born,
    sei_r_non_citizen, sei_r_rent, sei_r_space,
    ...
    sei_n_non_citizen, sei_n_rent, sei_n_space,
    
    # 5. Composite Heat Sensitivity Index
    csi_r, csi_n,
    
    # 6. Land Surface Temperature
    lst_median, lst_mean, grid_area_m2,
    lst_valid_area_m2, lst_coverage_pct,
...
output_grid_final <- file.path(
  interim_results_path,
  "berlin_grid_final.gpkg"
)
...
# =====================================================================
# Appendix table: Number of SEIs contributing to the CSI
# =====================================================================
...
# -----------------------------
# 2. Count number of available SEIs per grid cell
# -----------------------------
...
grid_final <- grid_final %>%
  mutate(
    n_indicators = rowSums(
      !is.na(
        as.data.frame(
          st_drop_geometry(.)[, sei_cols]
        )
      )
    )
  )
...
csi_indicator_coverage <- grid_final %>%
  st_drop_geometry() %>%
  count(n_indicators, name = "grid_cells") %>%
  mutate(
    share_grid_cells = round(
      grid_cells / sum(grid_cells) * 100,
      1
    )
  )

# View table
csi_indicator_coverage

# -----------------------------
# 4. Save appendix table
# -----------------------------
...
write_csv(
  ...
  file.path(
    appendix_path,
    "appendix_csi_indicator_coverage.csv"
  )
)